...
class Hivera {
  ...
  async runAccount() {
    ...
    log.info(`=========Tài khoản ${this.accountIndex + 1}| ${firstName + " " + lastName} | ${this.proxyIp} | Start later ${timesleep} second...`);
        ...
        workerPromises.push(
          new Promise((resolve) => {
            worker.on("message", (message) => {
              ...
              if (message.error) {
                // errors.push(`Account ${currentIndex + 1}: ${message.error}`);
                console.log(`Account ${message.accountIndex}: ${message.error}`);
              }
              resolve();
            });
            worker.on("error", (error) => {
              // errors.push(`Worker error for account ${currentIndex + 1}: ${error.message}`);
              console.log(`Worker error for account ${currentIndex + 1}: ${error.message}`);
              worker.terminate();
            });
            worker.on("exit", (code) => {
              worker.terminate();
              if (code !== 0) {
                // console.log(`Worker for the account ${currentIndex + 1} thoát với mã: ${code}`);
              }
              resolve();
            });
          })
